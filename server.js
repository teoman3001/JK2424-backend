...
// BU KISIM ZİNCİRLEME AKIŞIN OLDUĞU ESKİ HALİDİR
const STATUS_FLOW = {
...
// ZİNCİRLEME KONTROLÜ YAPAN ESKİ STATUS GÜNCELLEME YOLU
app.patch("/bookings/:id/status", (req, res) => {
  const { status: newStatus } = req.body;
  const idx = bookings.findIndex(b => b.id === req.params.id);
  if (idx === -1) return res.status(404).json({ success: false });
  
  // Eğer yeni statü STATUS_FLOW listesinde yoksa hata verir
  if (!(STATUS_FLOW[bookings[idx].status] || []).includes(newStatus)) {
